...
0.19.0. It is narrower than each backend's native capabilities.

## Compiler claims

| Package | Engine | Claimed profiles | Execution mode | Join collision policy |
|---|---|---|---|---|
| `etlantic-polars==0.20.0` | Polars | kernel `/1`, relational `/1`, `portable-string-advanced/1`, `portable-conversion/1`, `portable-statistics/1`, `portable-window/1`, `portable-complex-values/1`, `portable-complex-types/1`, `portable-reshape/1` | Eager and lazy | `fail` only |
| `etlantic-pyspark==0.20.0` | PySpark | kernel `/1`, relational `/1`, `portable-string-advanced/1`, `portable-conversion/1`, `portable-statistics/1`, `portable-window/1`, `portable-complex-values/1`, `portable-complex-types/1`, `portable-reshape/1` | Eager and lazy | `fail` only |
| `etlantic-pandas==0.20.0` | Pandas | kernel `/1`, relational `/1` | Eager only (`lazy=False`) | `fail` only |
| `etlantic-sql==0.20.0` | SQL | kernel `/1`, relational `/1` | Eager (relation/SQL) | `fail` only |
