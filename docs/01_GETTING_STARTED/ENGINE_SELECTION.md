...
|---|---|---|---|
| Learn the model in memory | `etlantic==0.20.0` | `development` | [Quickstart](QUICKSTART.md) |
| JSON / CSV files | core only | file storage bindings | [File storage](../06_EXECUTION/FILE_STORAGE_TUTORIAL.md) |
| Fast local dataframes | `etlantic[polars]==0.20.0` | `dataframe_engine="polars"` | [Polars tutorial](../06_EXECUTION/POLARS_TUTORIAL.md) |
| Pandas compatibility | `etlantic[pandas]==0.20.0` | `dataframe_engine="pandas"` | [Pandas tutorial](../06_EXECUTION/PANDAS_TUTORIAL.md) |
| Cross-engine Polars↔Pandas | `etlantic[dataframes]==0.20.0` | both plugins allowlisted | [Interchange example](../09_EXAMPLES/INTERCHANGE_POLARS_PANDAS.md) |
| Keep work in SQL | `etlantic[sql]==0.20.0` | `sql_engine="sql"` | [SQL tutorial](../06_EXECUTION/SQL_TUTORIAL.md) |
| Local Spark batch | `etlantic[pyspark]==0.20.0` | `spark_engine="pyspark"` | [PySpark tutorial](../06_EXECUTION/PYSPARK_TUTORIAL.md) |
| Emit Airflow DAGs | `etlantic[airflow]==0.20.0` | `orchestrator="airflow"` | [Airflow tutorial](../06_EXECUTION/AIRFLOW_TUTORIAL.md) |
| Prefect local scheduler | `etlantic[prefect]==0.20.0` | `orchestrator="prefect"` | [Prefect example](../09_EXAMPLES/PREFECT_RUN.md) |
...
2. **Pin the minor in 0.x.** For ETLantic 0.19, keep core and every official
   plugin on the same release (for example `etlantic==0.20.0` with
   `etlantic-polars==0.20.0`). Do not mix 0.18 plugins with 0.19 core.
